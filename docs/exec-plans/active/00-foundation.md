...
- [x] Firebase-project koppelen (auth + Firestore), credentials via environment-config, niet gecommit. Client-webconfig ingevuld in `src/environments/environment.ts` (echte `apiKey`/`appId`/... uit de Firebase-console, web-app "cardcase") — geen secret, zie beslissing hieronder over de gitleaks-allowlist die hiervoor nodig was.
...
- **gitleaks-allowlist voor de Firebase-apiKey**: `.gitleaks.toml` staat de `gcp-api-key`-detectie toe, maar uitsluitend voor het exacte `AIza...`-patroon in `src/environments/environment*.ts`. Firebase's client-side webconfig is geen secret (Firestore-rules zijn de echte toegangsgrens, zie `docs/governance/SECURITY.md`), maar matcht wel gitleaks' generieke Google-API-key-regel. Geen algemene uitzondering — alleen dit patroon, alleen op dit pad.
...
- **Firestore rules gedeployed** (2026-09-01): `firestore.rules` staat nu live op het echte project `cardcase-549a7`, door Julia zelf gedeployed (`npx firebase deploy --only firestore:rules`, na `npx firebase login`) en geverifieerd in de Firebase-console (Firestore Database → Rules). Deploy zelf bewust niet door de agent gedaan — raakt het echte, enige productieproject (zie `docs/governance/ENVIRONMENTS.md`), en werd ook geblokkeerd door de auto-mode classifier.
